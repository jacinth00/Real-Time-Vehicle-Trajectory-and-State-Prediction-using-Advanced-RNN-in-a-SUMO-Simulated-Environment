...
sumoCmd = [sumoBinary, "-c", "urban_grid.sumocfg"]
...
try:
    ...
    sys.exit("Error: Model file 'Levin_allcars_lstm.h5' not found. Please ensure it is in the same directory.")

# Manually recreate the StandardScaler with your values from training
...
while traci.simulation.getMinExpectedNumber() > 0:
    ...
    # Check if our AI-controlled vehicle is still in the simulation
    if vehicle_id in traci.vehicle.getIDList():
        ...
        try:
            ...
            # Fallback if geo-conversion is not set up in SUMO
            longitude, latitude = 0.0, 0.0

        # Create the full 13-feature vector for the model, using placeholders for non-simulated values
        features = np.array([[
        ...
        # Ensure predicted speed is not negative
        ...
        print(f"Step {step}: Current Speed={current_speed:.2f} m/s, Predicted Next Speed={predicted_speed:.2f} m/s")
